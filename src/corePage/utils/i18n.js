...
export function importAllLocales() {
    const modules = {};
    try {
        // 导入 @/project 下文件，包含子目录，文件名为：[/\\]locales[/\\]([a-z]{2})-?([A-Z]{2})?\.ts
        const projectRequireContext = require.context('../../project', true, /[/\\]locales[/\\]([a-z]{2})-?([A-Z]{2})?\.ts$/);
        ...
        const layoutsRequireContext = require.context('../layouts', true, /[/\\]locales[/\\]([a-z]{2})-?([A-Z]{2})?\.ts$/);
        ...
        const componentsRequireContext = require.context('../components', true, /[/\\]locales[/\\]([a-z]{2})-?([A-Z]{2})?\.ts$/);
        ...
        const localesRequireContext = require.context('../../locales', false, /([a-z]{2})-?([A-Z]{2})?\.ts$/);
