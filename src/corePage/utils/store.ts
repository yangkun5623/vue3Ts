 ...
 * 为自动导入的 store 做类型限制
 * [@/store文件夹下定义的 store]与[@/project文件夹下定义的`文件store.ts`] 必须继承此类型
 * @author LiQingSong
 */
...
export function importAllStore<S> (): ModuleTree<S> {
    ...
    try {
        ...
        const projectRequireContext: __WebpackModuleApi.RequireContext = require.context('../../project', true, /[/\\]store\.ts$/);
